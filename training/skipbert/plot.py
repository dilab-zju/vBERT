...
def _set_madvise(large_data, advise=1):
    ...
    assert madvise(large_data.ctypes.data, large_data.size * large_data.dtype.itemsize,
                   advise) == 0, "MADVISE FAILED"  # 1 means MADV_RANDOM
...
class Plot:
    def __init__(self, max_num_entries=100000, hidden_size=768):
        ...
        self.orig_trigram_hidden_states = _read_or_create_memmap("plot_hidden_states_tri_gram.memmap", dtype='float16',
                                                                 shape=(max_num_entries, 3, hidden_size))
