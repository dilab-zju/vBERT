...
from transformers.models.bert.modeling_bert import BertEmbeddings, BertEncoder, BertPooler, BertLayer, BertOnlyMLMHead
...
class BertSelfAttention(transformers.models.bert.modeling_bert.BertSelfAttention):
    ...
    def forward(
        ...
        outputs = (context_layer, attention_scores) if output_attentions else (
            context_layer,)  # hacked: replace attention_probs with attention_scores
...
class ShallowSkipping(nn.Module):
    ...
    def merge_ngrams(input_ids, ngram_hidden_states, aux_embeddings):
        this_states = ngram_hidden_states.clone()
        batch_size, seq_length = input_ids.shape
        lens = (input_ids != 0).sum(1)
        hidden_state = torch.zeros([batch_size, seq_length, this_states.size(-1)],
                                   dtype=this_states.dtype, device=this_states.device)

        # assert to be trigrams
        flat_hidden_state = this_states[:, 1]
        flat_hidden_state[:-1] = flat_hidden_state[:-1] + this_states[1:, 0]
        flat_hidden_state[1:] = flat_hidden_state[1:] + this_states[:-1, 2]
        k = 0
        ...
        # if ngram_hidden_states.equal(this_states):
        #     print(True)
        # else:
        #     print(False)
        return hidden_state
    ...
    def forward(
        ...
        if trigram_states is None:
            trigram_states = F.dropout(hidden_states, self.config.hidden_dropout_prob, self.training)
            # trigram_states = hidden_states
        else:
            trigram_states = F.dropout(trigram_states, self.config.hidden_dropout_prob, self.training)
        hidden_states = self.merge_ngrams(input_ids, trigram_states, aux_embeddings)
        hidden_states = model.norm(hidden_states)

        return hidden_states, trigram_states
...
class SkipBertModel(BertModel):
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings.position_embeddings2 = nn.Embedding(self.config.max_position_embeddings,
    ...
    def forward(
        ...
        hidden_states, tri_states = self.shallow_skipping(
            input_ids=input_ids,
            token_type_ids=token_type_ids,
            attention_mask=attention_mask,
            head_mask=head_mask,
        ...
        for i, layer_module in enumerate(self.encoder.layer[-self.config.num_full_hidden_layers:]):
            ...
            layer_head_mask = head_mask[
                i + self.config.num_hidden_layers - self.config.num_full_hidden_layers] if head_mask is not None else None
        ...
        return sequence_output, pooled_output, all_hidden_states, all_self_attentions, tri_states
...
class SkipBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        sequence_output, pooled_output, all_hidden_states, all_self_attentions, tri_states = self.bert(
            input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask,
            output_hidden_states=True, output_attentions=True, trigram_states=trigram_states)
...
class SkipBertForMaskedLM(BertForMaskedLM):
    def __init__(self, config, do_fit=False, share_param=True):
        super().__init__(config)
        ...
        self.hidden_size = config.hidden_size
        self.bert = SkipBertModel(config, add_pooling_layer=False)
    ...
    def rectify_input(self, input_ids, attention_mask):

        print("RECTIFY!!!")
        for x, y in zip(input_ids, attention_mask):
            sentence_length = torch.bincount(y)[1]
            pad_length = torch.bincount(x)[0]
            if sentence_length + pad_length == len(x):
                continue
            else:
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        try:
            sequence_output, pooled_output, all_hidden_states, all_self_attentions, trigram_states = self.bert(
                input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)
        except Exception as re:
            input_ids = self.rectify_input(input_ids, attention_mask)
            sequence_output, pooled_output, all_hidden_states, all_self_attentions, trigram_states = self.bert(
                input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)
...
class TestBertForMaskedLM(BertForMaskedLM):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.cls0 = BertOnlyMLMHead(config)

        # Initialize weights and apply final processing
        self.post_init()


class SkipBertForTokenClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, trigram_states=None):
        r"""
        ...
        sequence_output, pooled_output, all_hidden_states, all_self_attentions, tri_states = self.bert(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask,
            output_hidden_states=True, output_attentions=True, trigram_states=trigram_states)
