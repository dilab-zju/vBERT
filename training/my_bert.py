...
from transformers.adapters.mixins.bert import BertModelAdaptersMixin, BertOutputAdaptersMixin
from transformers.adapters.mixins.bert import BertSelfOutputAdaptersMixin
...
from skipbert.modeling import SkipBertModel, ShallowSkipping
...
class BertLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.has_adapter:
            ...
            self.hidden_size = 64
            if self.isLinear:
                self.adapter_fi = nn.Linear(self.in_size, self.hidden_size)
                self.adapter_se = nn.Linear(self.hidden_size, self.in_size)
            else:
    ...
    def forward(
        ...
        if self.has_adapter:
            ...
            if self.isLinear: 
                ...
                input_tensor = self.adapter_fi(attention_output)
                input_tensor = self.acfun(input_tensor)  # activation function
                input_tensor = self.adapter_se(input_tensor)
                attention_output = attention_output + input_tensor  # residual connection
                attention_output = attention_output.view([batch_num, line_num, self.in_size])
            else:  
                if layer_adapter_param is not None:  
                    input_tensor = torch.bmm(attention_output,
                                             torch.transpose(input=layer_adapter_param[0], dim0=2, dim1=1)[0:batch_num])
                    input_tensor = input_tensor + layer_adapter_param[1].unsqueeze(1).repeat(1, line_num, 1)[
                                                  0:batch_num]

                    input_tensor = self.acfun(input_tensor)  # activation function

                    input_tensor = torch.bmm(input_tensor,
                                             torch.transpose(input=layer_adapter_param[2], dim0=2, dim1=1)[0:batch_num])
                    input_tensor = input_tensor + layer_adapter_param[3].unsqueeze(1).repeat(1, line_num, 1)[
                                                  0:batch_num]
...
class BertEncoder(nn.Module):
    ...
    def forward(
            self,
            hidden_states: torch.Tensor,
            adapter_param: Optional[Tuple[torch.Tensor]] = None,
            attention_mask: Optional[torch.FloatTensor] = None,
            head_mask: Optional[torch.FloatTensor] = None,
            encoder_hidden_states: Optional[torch.FloatTensor] = None,
            encoder_attention_mask: Optional[torch.FloatTensor] = None,
        ...
        for i, layer_module in enumerate(self.layer):
            if adapter_param is not None:
                layer_adapter_param = adapter_param[4 * i:4 * i + 4]
            else:
                layer_adapter_param = None
...
class BertAdapterForSequenceClassification(ModelWithHeadsAdaptersMixin, BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        if adapter_param is not None:  # 采用传参的方式进行计算
            pooled_output = pooled_output.unsqueeze(1)
            batch_num = pooled_output.size(0)
            pooled_output = torch.bmm(pooled_output,
                                      torch.transpose(input=adapter_param[-2], dim0=2, dim1=1)[0:batch_num])
            logits = pooled_output + adapter_param[-1].unsqueeze(1).repeat(1, 1, 1)[0:batch_num]
...
class SkipBertAdapterForSequenceClassification(ModelWithHeadsAdaptersMixin, BertPreTrainedModel):
